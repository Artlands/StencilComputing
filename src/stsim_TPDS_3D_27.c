...
int main(int argc, char* argv[])
{
  ...
  int sten_order = 1;          // stencil order,refers to the number of elements in each dimension, not counting the element at the intersection
  int sten_ptnum = 27;          // stencil computing elements
  ...
  uint64_t cache_size = 32;             // default cache size, in KB
...
/*----------------------------------------------------------------------------*/
  ...
  while( (ret = getopt( argc, argv, "x:y:z:o:i:p:c:b:v:a:h")) != -1 )
  {
    switch( ret )
    {
      ...
      case 'h':
        ...
        printf(" -v <Host cache size: 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192>");
        printf(" h ...print help\n");
        return 0;
        break;
      default:
  ...
  sprintf(tracelog, "../traces/trace_TPDS_3D_27.log");
  ...
  sprintf(cachelog, "../traces/cache_TPDS_3D_27.log");
  ...
  cache_node_32 host_cache = {.cache_size = cache_size * 1024,
                              .block_size = 64,
                              .ways = 8,
                              .sets = 64,
                              .hits = 0,
  ...
  if( flag == 1 )
  {
    sprintf(filename, "../traces/PIMS-%dD-%dpoints-O%dX%dY%dZ%d_TPDS_3D_27",
            dim, sten_ptnum, sten_order, dim_x, dim_y, dim_z);
  }
  else
  {
    sprintf(filename, "../traces/HOST-%dD-%dpoints-O%dX%dY%dZ%d_TPDS_3D_27",
            dim, sten_ptnum, sten_order, dim_x, dim_y, dim_z);
  }
#ifdef GENTRACE
      ...
      for( j = sten_order/2; j < (dim_y+sten_order/2); j++)
      {
        ...
        for( i = sten_order/2; i < (dim_x+sten_order/2); i++ )
        {
          ...
          if( flag == 0 )
          {
            ...
            // Read its neighbor points
            ret = host_lru_simulation(&host_cache, v_pointer[i-1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i-1]);
#endif
            }
            total_HOST_RD++;
            ...
            // dim_y
            ret = host_lru_simulation(&host_cache, v_pointer[i - 1 * tdim_x]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - 1 * tdim_x]);
#endif
            }
            total_HOST_RD++;

            ret = host_lru_simulation(&host_cache, v_pointer[i - 1 * tdim_x + 1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - 1 * tdim_x + 1]);
#endif
            }
            total_HOST_RD++;
            ...
            // dim_z
            ret = host_lru_simulation(&host_cache, v_pointer[i - 1 * tdim_x * tdim_y]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - 1 * tdim_x * tdim_y]);
#endif
            }
            total_HOST_RD++;

            ret = host_lru_simulation(&host_cache, v_pointer[i - 1 * tdim_x * tdim_y - 1 * tdim_x]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - 1 * tdim_x * tdim_y - 1 * tdim_x]);
#endif
            }
            total_HOST_RD++;

            ret = host_lru_simulation(&host_cache, v_pointer[i - 1 * tdim_x * tdim_y - 1 * tdim_x + 1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - 1 * tdim_x * tdim_y - 1 * tdim_x + 1]);
#endif
            }
            total_HOST_RD++;
            ...
            ret = host_lru_simulation(&host_cache, v_pointer[i - 1 * tdim_x * tdim_y + 1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - 1 * tdim_x * tdim_y + 1]);
#endif
            }
            total_HOST_RD++;
            ...
            // First group
            // v_point = [i-1], [i+1], [i + tdim_x], [i - tdim_x], [i - tdim_x * tdim_y], [i + tdim_x * tdim_y]
            memset(ops, 0, sizeof(ops));
            sprintf(ops, "HOST_LA");
#ifdef GENTRACE
            write_to_file(tracefile, ops, stor_size, v_pointer[i]);
            ...
            getpimsid( &pimsid, v_pointer[i-1], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i-1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i-1]);
            ...
            getpimsid( &pimsid, v_pointer[i + tdim_x], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i + tdim_x]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i + tdim_x]);
#endif
              total_DRAM_RD++;
            }
            total_PIMS_RD++;
            ...
            getpimsid( &pimsid, v_pointer[i + tdim_x * tdim_y], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i + tdim_x * tdim_y]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i + tdim_x * tdim_y]);
#endif
              total_DRAM_RD++;
            }
            total_PIMS_RD++;
            ...
            // Second group
            // v_point = [i + tdim_x - 1], [i + tdim_x + 1], [i - tdim_x - 1], [i - tdim_x + 1]
            // [i - tdim_x * tdim_y + 1], [i - tdim_x * tdim_y - 1], [i - tdim_x * tdim_y + tdim_x], [i - tdim_x * tdim_y - tdim_x]
            // [i + tdim_x * tdim_y + 1], [i + tdim_x * tdim_y - 1], [i + tdim_x * tdim_y + tdim_x], [i + tdim_x * tdim_y - tdim_x]
            memset(ops, 0, sizeof(ops));
            sprintf(ops, "HOST_LA");
#ifdef GENTRACE
            ...
            getpimsid( &pimsid, v_pointer[i + tdim_x - 1], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i + tdim_x - 1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i + tdim_x - 1]);
#endif
              total_DRAM_RD++;
            }
            total_PIMS_RD++;
            ...
            getpimsid( &pimsid, v_pointer[i - tdim_x * tdim_y + 1], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i - tdim_x * tdim_y + 1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - tdim_x * tdim_y + 1]);
#endif
              total_DRAM_RD++;
            }
            total_PIMS_RD++;
            ...
            getpimsid( &pimsid, v_pointer[i - tdim_x * tdim_y + tdim_x], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i - tdim_x * tdim_y + tdim_x]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - tdim_x * tdim_y + tdim_x]);
#endif
              total_DRAM_RD++;
            }
            total_PIMS_RD++;
            ...
            // Thrid group
            // v_point = [i - tdim_x * tdim_y + tdim_x + 1], [i - tdim_x * tdim_y + tdim_x - 1], [i - tdim_x * tdim_y - tdim_x + 1], [i - tdim_x * tdim_y - tdim_x - 1]
            // [i + tdim_x * tdim_y + tdim_x + 1], [i + tdim_x * tdim_y + tdim_x - 1], [i + tdim_x * tdim_y - tdim_x + 1], [i + tdim_x * tdim_y - tdim_x - 1]
            memset(ops, 0, sizeof(ops));
            sprintf(ops, "HOST_LA");
#ifdef GENTRACE
            write_to_file(tracefile, ops, stor_size, v_pointer[i]);
            ...
            getpimsid( &pimsid, v_pointer[i - tdim_x * tdim_y + tdim_x + 1], shiftamt, vaults );
            ret = pims_lru_simulation(&pims_cache[pimsid], v_pointer[i - tdim_x * tdim_y + tdim_x + 1]);
            if( ret == -1 )
            {
#ifdef GENTRACE
              write_to_file(tracefile, ops, stor_size, v_pointer[i - tdim_x * tdim_y + tdim_x + 1]);
#endif
              total_DRAM_RD++;
            }
            total_PIMS_RD++;

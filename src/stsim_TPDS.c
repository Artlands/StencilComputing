...
/*----------------------------------------------------------------------------*/
  ...
  sprintf(tracelog, "../traces/trace_TPDS.log");
  ...
  sprintf(cachelog, "../traces/cache_TPDS.log");
  ...
  if( flag == 1 )
  {
    sprintf(filename, "../traces/PIMS-%dD-%dpoints-O%dX%dY%dZ%d_TPDS",
            dim, sten_ptnum, sten_order, dim_x, dim_y, dim_z);
  }
  else
  {
    sprintf(filename, "../traces/HOST-%dD-%dpoints-O%dX%dY%dZ%d_TPDS",
            dim, sten_ptnum, sten_order, dim_x, dim_y, dim_z);
  }
#ifdef GENTRACE
 ...
 #ifdef GENTRACE
              // By pass the host cache -TPDS2020-revise
               write_to_file(tracefile, ops, stor_size, v_pointer[i]);
 #endif
               total_HOST_LA++;
